 ...
 * optionid,
 * optionhomeTeam.id, optionhomeTeam.name, optionhomeTeam.image,
 * optionawayTeam.id, optionawayTeam.name, optionawayTeam.image,
 */
...
const updateImageURL = (asset) => {
  ...
  const pattern = /img.domainone.com\/cms/g;
  const replaceWith = 'img.domaintwo.com';
  asset = JSON.parse(JSON.stringify(asset).replace(pattern, replaceWith));
  return asset;
};

resultOutput = exampleInputs
